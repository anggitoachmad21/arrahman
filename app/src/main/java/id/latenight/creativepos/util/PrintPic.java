...
public class PrintPic {
    ...
    public void drawImage(float x, float y, Bitmap btm) {
        try {
            float centreX = (this.canvas.getWidth() - btm.getWidth()) / 2;
            Log.e("X", String.valueOf(centreX));
            float centreY = y;
            this.canvas.drawBitmap(btm, centreX, centreY, new Paint(Paint.ANTI_ALIAS_FLAG));
